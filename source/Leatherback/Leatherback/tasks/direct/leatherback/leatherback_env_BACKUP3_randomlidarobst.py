...
class LeatherbackSceneCfg(InteractiveSceneCfg):
    ...
    # Note: No filter_prim_paths_expr since obstacles are created after scene setup
    # We handle filtering in code by checking force magnitude and contact bodies
...
class LeatherbackEnv(DirectRLEnv):
    ...
    def _check_obstacle_collisions(self):
        """Check if the robot has collided with any obstacles using contact sensors.
        Filters ground contact by checking contact force magnitude on chassis sensor only."""
        collision_detected = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
        
        # Only check chassis sensor for obstacle collision (not wheels, which always touch ground)
        if hasattr(self.scene, 'sensors') and 'contact_chassis' in self.scene.sensors:
            contact_sensor = self.scene.sensors['contact_chassis']
            ...
            if hasattr(contact_sensor, 'data') and hasattr(contact_sensor.data, 'net_forces_w'):
                ...
                if contact_data.net_forces_w is not None:
                    # Get force magnitude for each environment
                    force_magnitudes = torch.norm(contact_data.net_forces_w, dim=-1)  # Shape: (num_envs, num_bodies)
                    max_forces = torch.max(force_magnitudes, dim=1)[0]  # Max force per env
                    
                    # High threshold: chassis should NOT touch anything during normal driving
                    # Any chassis contact > 30N indicates obstacle collision (ground contact shouldn't happen)
                    collision_threshold = 30.0  # 30N threshold for chassis
                    collision_detected = max_forces > collision_threshold
                    
                    # Debug output for collisions
                    if torch.any(collision_detected):
                        for env_idx in range(self.num_envs):
                            if collision_detected[env_idx]:
                                print(f"[COLLISION] Environment {env_idx}: Chassis hit obstacle! Force: {max_forces[env_idx]:.3f}N")
    ...
    def _create_all_obstacles_at_setup(self):
        ...
        This is required for contact sensors to properly size their CUDA buffers."""
        print(f"[DEBUG] Creating obstacles for {self.num_envs} environments at scene setup...")
        
        # Initialize obstacle tensors if not already done (they're needed during scene setup)
        if self._obstacle_sizes is None:
            self._obstacle_sizes = torch.zeros((self.num_envs, 2, 3), device=self.device, dtype=torch.float32)
        if self._obstacle_positions is None:
            self._obstacle_positions = torch.zeros((self.num_envs, 2, 3), device=self.device, dtype=torch.float32)
        
        # Create 2 test obstacles per environment
        for env_idx in range(self.num_envs):
            env_origin = self.scene.env_origins[env_idx]
            
            for obs_idx in range(2):  # 2 obstacles per env
                ...
                # Initial position RELATIVE to environment (prim is under /World/envs/env_X/)
                # Start at origin of the environment coordinate system
                initial_pos = (0.0, 0.0, 0.55)
                obstacle_cfg.func(prim_path, obstacle_cfg, translation=initial_pos)
        
        print(f"[DEBUG] Created {self.num_envs * 2} obstacles at scene setup")
    ...
    def _debug_contact_sensors(self):
        ...
        if hasattr(self.scene, 'sensors'):
            ...
            expected_sensors = ['contact_chassis', 'contact_wheel_front_left', 'contact_wheel_front_right', 'contact_wheel_rear_right']
